...
const newsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case FETCHING_NEWS_FAILURE:
      return {
        ...state, isRefreshing: false, isLoading: false, errorMessage: action.payload,
      };

    case FETCHING_NEWS_SUCCESS:
      return {
        ...state, isRefreshing: false, isLoading: false, data: action.payload, page: 3,
      };

    case LOAD_MORE_NEWS_SUCCESS:
      // Increasing the number of pages by 2 each time since
      // we are getting 20 news each time (10 News a page)
      // eslint-disable-next-line no-case-declarations
      const vPage = state.page + 2;
